...
public class JdbcProductRepository implements ProductRepository {
	...
	public List<Product> listAll() {
		final String SELECT_ALL_PRODUCT = "SELECT p.id, p.name, p.unit, p.provider_id, v.name as provider_name   " + 
				"FROM product p  " + 
	...
	public Product findById(int id) {
		final String SELECT_PRODUCT_BY_ID ="SELECT p.id, p.name, p.unit, p.provider_id, v.name as provider_name   " + 
				"FROM product p  " + 
				"	join provider v on (p.provider_id=v.id)   " +
	...
	public Product add(Product newProduct) {
		final String INSERT_NEW_PRODUCT = "INSERT INTO product(name, unit, provider_id) "
				+ "VALUES (?, ?, ?)";
	...
	public Product update(int id, Product updatedProduct) {
		final String UPDATE_PRODUCT = "UPDATE product SET name=?, unit=?, provider_id=? "
				+ " WHERE id=?";
